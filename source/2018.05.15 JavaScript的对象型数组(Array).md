...
var b = Array(1,2);
```
![](https://i.loli.net/2018/05/15/5afa9acaa6b07.png)

用`Array`来声明数组有一个很大的问题，就是参数的不同会导致不同的结果
**1个参数**
```JavaScript
//空参数
new Array(); //定义一个空数组 []
...
```
![](https://i.loli.net/2018/05/15/5afaa3fa81d71.png)
从内存图可以看出，数组的`__proto__`是指向`Array.prototype`的，而伪数组的`__proto__`是直接指向`Object.prototype`
...
```
其中`this`是可选参数，`function`的`array`参数在某些情况下也可以不写
`forEach()`的用处是将数组中的每一组值以`value-key`的形式传递给`function`，调用函数执行一次
例如
```JavaScript
var arr =  [1,2,3,4,5];

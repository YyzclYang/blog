...
它也是用一个对象模拟"类"。在这个类里面，定义一个构造函数createNew()，用来生成实例。然后，在createNew()里面，定义一个实例对象，将私有属性和方法直接写在实例对象上面，将共有属性和方法写在实例对象的`__proto__`上面，避免共有属性的重复定义，浪费内训。最后把这个实例对象作为返回值。使用的时候，调用createNew()方法，就可以得到实例对象。
